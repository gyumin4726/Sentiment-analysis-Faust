...
from LSTM.predict_lstm import predict_sentiment
...
# 감정 점수 이진화: joy/love → 1, sadness/anger/fear → 0
...
process_scene(
    ...
    title="계약 장면 감정 흐름 비교 (LSTM)",
    save_name="scene_contract_emotion_continuous_flow_lstm.png"
)

process_scene(
    faust_path="../faust_XXIII.txt",
    mephi_path="../mephi_XXIII.txt",
    title="Dreary Day 장면 감정 흐름 비교 (LSTM)",
    save_name="scene_drearyday_emotion_continuous_flow_lstm.png"
)

process_scene(
    faust_path="../faust_XXV.txt",
    mephi_path="../mephi_XXV.txt",
    title="감옥 장면 감정 흐름 비교 (LSTM)",
    save_name="scene_prison_emotion_continuous_flow_lstm.png"
)
